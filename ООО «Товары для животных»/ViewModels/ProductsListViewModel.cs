...
internal class ProductsListViewModel : ViewModel
{
    ...
    public ProductsListViewModel(MainViewModel mainViewModel, IObjectsActions objectsActions)
    {
        this.mainViewModel = mainViewModel;

        try
        {
            Manufacturers = objectsActions.GetManufacturesToList();
        }
        catch
        {
        ...
        RemoveProductCommand = new(() =>
        {
            ...
            try
            {
                objectsActions.RemoveProduct(SelectedProduct); 
    ...
    private void SearchForProducts()
    {
        try
        {
            //Вопрос к Алексею Анатольевичу. Так можно или нет
            IObjectsActions objectsToList = new ObjectsActionsClass();

            var countAll = objectsToList.GetProductsCount(); 

            var productsQuery = objectsToList.SearchProduct(SearchText);  
