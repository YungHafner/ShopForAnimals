...
internal class EditProductViewModel : ViewModel
{
    ...
    public EditProductViewModel(Product edit, MainViewModel mainViewModel, IObjectsActions objectsToList, IEditProductEntry editProductEntry)
    {
        original = edit;

        CanEditArticle = !string.IsNullOrEmpty(edit.ProductArticleNumber);

        Manufacturers =  objectsToList.GetManufacturesToList();
        Categories = objectsToList.GetCategoriesToList(); 
        Providers = objectsToList.GetProvidersToList();
        ...
        Save = new Command(() =>
        {
            try
            {
                ...
                editProductEntry.SaveEditProduct(original, EditProduct); 
